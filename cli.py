...
def merge_nasal_phonemes(
        ...
        """
        Merge sequences like ('a', '~') into nasalized phonemes like 'ã'.
        """
        merged = []
        i = 0
        while i < len(segments):
            ...
            # Check if next segment is a nasal diacritic
            if i + 1 < len(segments):
                next_start, next_end, next_label = segments[i + 1]
                next_label = next_label.strip()
                
                if next_label == '~' and label in nasal_merge_map:
                    nasal = nasal_merge_map[label]
                    merged.append((start, next_end, nasal))
                    i += 2
                    continue
...
def ctc_kreuk_pipeline(
    ...
    output_path = os.path.join("outputs", f"{base_name}_seg.TextGrid")
